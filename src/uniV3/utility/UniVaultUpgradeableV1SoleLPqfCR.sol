...
contract UniVaultUpgradeableV1SoleLPqfCR is
    ERC20Upgradeable,
    ...
    ControllableInit
{
    ...
    function _migrateLiquidity(uint128 liquidityAmount, uint256 newPosId, uint24 swapInPoolwithFee) internal {
        ...
        if (swapInPoolwithFee != 0) {
            getStorage().setFee(swapInPoolwithFee);
        }
        _zap();
        if (swapInPoolwithFee != 0) {
    ...
    function injectLiquidity(uint256 inject0, uint256 inject1) public {
        if (inject0 != 0) {
            token0().safeTransferFrom(msg.sender, address(this), inject0);
        }

        if (inject1 != 0) {
            token1().safeTransferFrom(msg.sender, address(this), inject1);
        }
    ...
    function changeRange(uint256 newPosId, uint128 loopNum, uint24 swapInPoolWithFee, uint256 inject0, uint256 inject1)
        public
        onlyGovernance
    {
        ...
        require(_t0 == address(token0()) && _t1 == address(token1()) && _fee == getStorage().fee(), "pool mismatch");

        //uint256 oldPosId = getStorage().posId();

        // set new ranges so that the zap in migrate will work
        ...
        // then lets inject liquidity and sweep the rest.
        injectLiquidity(inject0, inject1);
    }
